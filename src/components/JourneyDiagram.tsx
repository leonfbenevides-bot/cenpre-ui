  ...
  return (
    <div className={cn("flex items-center gap-3", align === "right" && "flex-row-reverse")}>
      <div className="flex h-[72px] w-[72px] shrink-0 flex-col items-center justify-center gap-1.5 rounded-chip border-2 border-ash-200 bg-white">
        <span className="text-magenta-700" aria-hidden>{icon}</span>
        <span className="rounded-chip bg-ash-200 px-1.5 py-0.5 text-[11px] font-medium text-charcoal-400">{label}</span>
      </div>
      <div
...
export function JourneyDiagram({ aluno, empresa, className }: JourneyDiagramProps) {
  return (
    <div className={cn("relative overflow-hidden rounded-[24px] border border-ash-200 bg-white p-6 md:p-10", className)}>
      <div className="grid grid-cols-1 items-center gap-8 md:grid-cols-[1fr_auto_1fr] md:gap-6">
        <div className="flex flex-col gap-4">
          ...
          {aluno.nos.map((n) => <NodeChip key={n.label} {...n} align="right" />)}
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {empresa.nos.map((n) => <NodeChip key={n.label} {...n} align="left" />)}
        </div>
      </div>
    </div>
  );
