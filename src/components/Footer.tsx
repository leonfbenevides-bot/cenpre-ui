...
export interface FooterProps {
  brand?: ReactNode;
  /** Ação abaixo da marca (ex.: "Acessar UCAM portal"). */
  ...
  legal?: ReactNode;
  /** Ícones de redes sociais, exibidos ao lado da linha legal. */
  social?: ReactNode;
  className?: string;
}
      ...
      <div className="mx-auto max-w-container px-6 py-14 md:px-gutter">
        <div className="grid gap-x-8 gap-y-10 sm:grid-cols-2 lg:[grid-template-columns:1.4fr_1fr_1fr_1fr_1.4fr]">
          ...
          {columns.map((col) => (
            <div key={col.title} className="flex flex-col gap-3">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-ash-400">
                {col.title}
              </h3>
              <ul className="flex flex-col gap-2">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <a
                      href={link.href}
                      className="text-sm text-ash-300 transition-colors hover:text-white"
                    >
                      {link.label}
                    </a>
                  </li>
        ...
        {(legal || social) && (
          <div className="mt-12 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-6">
            {legal && <div className="text-xs text-ash-400">{legal}</div>}
