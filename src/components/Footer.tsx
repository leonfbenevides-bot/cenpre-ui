...
export function Footer({ brand, columns, contact, legal, className }: FooterProps) {
  return (
    <footer className={cn("bg-charcoal-500 text-ash-300", className)}>
      <div className="mx-auto max-w-container px-6 py-14 md:px-[72px]">
        <div className="grid gap-x-8 gap-y-10 sm:grid-cols-2 lg:[grid-template-columns:1.4fr_1fr_1fr_1fr_1.4fr]">
          ...
          {columns.map((col) => (
            <div key={col.title} className="flex flex-col gap-3">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-ash-600">{col.title}</h3>
          ...
          {contact && (
            <div className="flex flex-col gap-3">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-ash-600">Contato</h3>
              <div className="text-sm leading-relaxed text-ash-400">{contact}</div>
            </div>
          )}
        </div>
        {legal && <div className="mt-12 border-t border-white/10 pt-6 text-xs text-ash-600">{legal}</div>}
      </div>
    </footer>
  );
}
